...
(function () {
    ...
    // Fork: unique SCRIPT_ID so this script can coexist with the official version
    // (separate DOM ids, CSS classes, keyframes, and localStorage keys).
    const SCRIPT_ID = 'douyin-downloader-pro';
    // Official version's panel id, used to shift our floating button out of its way.
    const OFFICIAL_PANEL_ID = 'douyin-downloader-panel';
    const OFFICIAL_EDGE_GAP = 78;
    ...
    // Fork: starts edge-aligned like the official button; shifts left when the official panel is detected.
    let PANEL_EDGE_OFFSET = 16;
    ...
    const style = `
        #${PANEL_ID} {
            ...
            /* Fix: let mouse events pass through the panel's invisible area (the reserved status-bubble
               space on the left of the toggle) so Douyin's own buttons (like, comments, etc.) stay
               clickable. Only the toggle button below re-enables pointer-events. */
            pointer-events: none;
        }
        ...
        #${PANEL_TOGGLE_ID} {
            ...
            touch-action: none;
            /* Keep the toggle button itself fully interactive (click + vertical drag). */
            pointer-events: auto;
        }
    ...
    // Fork: detect the official script's floating panel and shift ours to the left
    // so the two buttons do not overlap when both scripts are installed.
    function watchForOfficialPanel() {
        const reposition = () => {
            if (document.getElementById(OFFICIAL_PANEL_ID)) {
                PANEL_EDGE_OFFSET = OFFICIAL_EDGE_GAP;
                applyPanelPosition();
                return true;
            }
            return false;
    ...
    function applyPanelPosition() {
        ...
        panel.style.right = `${PANEL_EDGE_OFFSET}px`;
    }
    ...
    async function downloadVideoUrl(videoUrl, filename, onProgress, options = {}) {
        ...
        if (!isBlobUrl && !directoryHandle && typeof GM_download === 'function') {
            try {
                ...
                console.warn('[Douyin Downloader Pro] GM_download failed, falling back to GM_xmlhttpRequest.', error);
            }
        }
        ...
        if (!isBlobUrl && !directoryHandle) {
            try {
                ...
                console.warn('[Douyin Downloader Pro] GM_xmlhttpRequest download failed, falling back to fetch.', error);
            }
        }
        ...
        if (directoryHandle) {
            ...
            try {
                ...
                console.warn('[Douyin Downloader Pro] Directory download via GM_xmlhttpRequest failed, falling back to fetch.', error);
            }
        }
        ...
        try {
            ...
            if (directoryHandle && typeof GM_download === 'function') {
                console.warn('[Douyin Downloader Pro] Directory download via fetch failed, falling back to browser download.', error);
                await gmDownload(videoUrl, filename, onProgress);
                return;
            }
    ...
    function normalizeVideoPageUrl(href) {
        ...
        try {
            ...
            console.warn('[Douyin Downloader Pro] Failed to normalize profile link.', error);
        }
    ...
    async function resolveRecommendVideoEntry(video) {
        ...
        if (activeVideoId) {
            ...
            try {
                ...
                console.warn('[Douyin Downloader Pro] Recommend video page resolution failed, falling back.', error);
            }
            ...
            try {
                ...
                console.warn('[Douyin Downloader Pro] Recommend aweme detail resolution failed.', error);
            }
        }
    ...
    async function resolveCurrentVideoEntry() {
        ...
        if (locationVideoId && scopedPage) {
            try {
                ...
                console.warn('[Douyin Downloader Pro] Scoped page resolution failed, falling back.', error);
            }
        }
        ...
        if (feedStylePage && activeVideoId) {
            ...
            try {
                ...
                console.warn('[Douyin Downloader Pro] Feed video page resolution failed, falling back.', error);
            }
        }
        ...
        if (feedStylePage && activeVideoId) {
            try {
                ...
                console.warn('[Douyin Downloader Pro] Feed aweme detail resolution failed, falling back.', error);
            }
        }
        ...
        try {
            ...
            for (let index = 0; index < candidateUrls.length; index += 1) {
                try {
                    ...
                    console.warn('[Douyin Downloader Pro] Candidate video URL failed.', error);
                }
            }
    ...
    async function buildBatchEntriesFromLinks(links) {
        ...
        for (let index = 0; index < links.length; index += 1) {
            ...
            try {
                ...
                console.error('[Douyin Downloader Pro] Batch entry resolve failed.', pageUrl, error);
                entries.push({
                    id: `entry-${index}-${Date.now()}`,
                    pageUrl,
                    videoUrl: '',
